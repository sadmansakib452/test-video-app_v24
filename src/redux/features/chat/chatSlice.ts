...
interface ChatState {
  ...
  typingUsers: Record<string, TypingUserProfile | null>; // conversationId -> user profile or null
  authError: boolean;
}
  ...
  async (
    {
      ...
      conversationId,
      message,
    }: { receiverId: string; conversationId: string; message: string },
...
export const createConversation = createAsyncThunk(
  ...
  async (
    { creatorId, participantId }: { creatorId: string; participantId: string },
  ...
  async (
    {
      receiverId,
      conversationId,
    }: { receiverId: string; conversationId: string },
    { rejectWithValue, dispatch }
  ) => {
    try {
      // Make sure chat is initialized
    ...
    builder
      ...
      .addCase(fetchConversations.fulfilled, (state, action) => {
        ...
        if (state.activeConversation) {
          const updatedConversation = action.payload.find(
      ...
      .addCase(sendTypingStatus.fulfilled, (state, action) => {
        if (action.payload) {
          state.typingUsers[action.payload.conversationId] = null;
        }
      })
