...
export function ChatProvider({ children }: { children: ReactNode }) {
  ...
  const refreshConversations = async () => {
    try {
      ...
      if (activeConversation) {
        const updatedConversation = conversationsData.find((c) => c.id === activeConversation.id)
  ...
  const sendMessage = async (message: string) => {
    ...
    try {
      ...
      const receiverId =
        activeConversation.creator_id === user.id ? activeConversation.participant_id : activeConversation.creator_id
  ...
  const startNewConversation = async (userId: string) => {
    ...
    try {
      setIsLoading(true)
      const conversation = await chatService.createConversation(user.id, userId)
